...
for learning_rate in learning_rates:
    np.random.seed(1)
    network = SynapticCacheNetwork(layers, activation_function, learning_rate, eLTP_upkeep, decay_rate, scheme, 0.007081527394386207)      
    #%% Training
    epochs, accuracies, energies = network.train_until(95, x_train, y_train, n_samples_in_batch)
    energies_all.append(energies)
    accuracies_all.append(accuracies)
    
with open('fig_energy_vs_accuracy_synsyn_fixed_threshold.csv', 'w') as file:
